  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        <div className="bg-white p-2.5 rounded-2xl text-[#1877F2] border-2 border-indigo-500 shadow-lg">
      ...
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-black text-indigo-100 uppercase tracking-[0.2em] ml-1 mb-2 block">
          ...
          {selectedImage ? (
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 px-5 py-4 rounded-2xl border-2 border-indigo-400/50 bg-white text-sm font-black text-slate-900 placeholder:text-slate-400 focus:ring-4 focus:ring-indigo-500/20 outline-none transition-all shadow-xl" 
              />
              <button 
                type="button" 
                onClick={() => fileInputRef.current?.click()} 
                className="w-16 bg-white border-2 border-indigo-400 rounded-2xl text-indigo-600 flex items-center justify-center hover:bg-indigo-50 transition-all shadow-xl text-2xl active:scale-95"
              >
                📷
              </button>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-black text-indigo-100 uppercase tracking-widest ml-1 block">Pricing</label>
            <input 
              value={price} 
              onChange={(e) => setPrice(e.target.value)} 
              placeholder="ဥပမာ- ၅၅၀၀ ကျပ်" 
              className="w-full px-5 py-3.5 rounded-2xl border-2 border-indigo-400/50 bg-white text-sm font-black text-slate-900 placeholder:text-slate-400 outline-none shadow-lg focus:ring-4 focus:ring-indigo-500/20 transition-all" 
            />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-black text-indigo-100 uppercase tracking-widest ml-1 block">Phone</label>
            <input 
              value={phone} 
      ...
      <button 
        type="submit" 
        disabled={isLoading} 
        className="group relative w-full py-5 bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-black rounded-[1.5rem] shadow-2xl transition-all border-b-4 border-indigo-900/50 uppercase text-sm tracking-widest active:scale-[0.97] active:border-b-0 disabled:bg-slate-400 overflow-hidden"
      >
      ...
      <p className="text-[10px] text-center text-indigo-100 font-bold uppercase tracking-tighter opacity-80">
