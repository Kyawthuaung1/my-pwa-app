...
import { generateMarketingContent, generateProductVisual } from './geminiService';
...
const App: React.FC = () => {
  ...
  const handleFormSubmit = async (input: { 
    ...
    try {
      addLog("အချက်အလက်များကို AI ဖြင့် စတင်စစ်ဆေးနေပါသည်...");
      const plan = await generateMarketingContent(input);
      setMarketingPlan(plan);
      setCurrentProduct(plan.productName);
      addLog("အရောင်းစာသားများ ဖန်တီးမှု အောင်မြင်ပါသည်။");

      // AUTOMATIC IMAGE GENERATION - Don't wait for user to click
      if (!input.image) {
        addLog("Product Visual ဖန်တီးနေပါသည်...");
        try {
          const visualUrl = await generateProductVisual(plan.productName);
          setCurrentImageUrl(visualUrl);
          addLog("Product Visual ဖန်တီးမှု ပြီးမြောက်ပါပြီ။");
          saveToHistory(plan, input.link || 'Input', visualUrl);
        } catch (imgErr) {
          addLog("ဓာတ်ပုံဖန်တီးမှု ခေတ္တအဆင်မပြေဖြစ်နေပါသည်။");
          saveToHistory(plan, input.link || 'Input', undefined);
        }
      } else {
        // Use the uploaded image
        const uploadedUrl = `data:${input.image.mimeType};base64,${input.image.data}`;
        setCurrentImageUrl(uploadedUrl);
        saveToHistory(plan, 'Image Upload', uploadedUrl);
      }
      
      if (userEmail) {
        setLoadingState(LoadingState.SENDING_GMAIL);
        addLog(`Gmail Report (${userEmail}) သို့ ပေးပို့နေပါသည်...`);
        await new Promise(resolve => setTimeout(resolve, 500));
      }

      setLoadingState(LoadingState.COMPLETE);
      addLog("အရာအားလုံး ပြီးမြောက်ပါပြီ။");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      <nav className="bg-white/90 backdrop-blur-xl border-b px-8 py-4 flex justify-between items-center sticky top-0 z-[100] shadow-sm">
      ...
      <main className="max-w-7xl mx-auto w-full p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 relative">
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-[#1877F2] p-8 rounded-[2.5rem] shadow-2xl border-4 border-indigo-400/30">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          {(marketingPlan || loadingState === LoadingState.ANALYZING) ? (
            <div className="space-y-8 animate-in fade-in duration-700">
              {marketingPlan && <ContentDisplay plan={marketingPlan} />}
              <ImageGenerator 
            ...
            <div className="h-full min-h-[500px] bg-white/40 backdrop-blur-lg rounded-[4rem] border-2 border-dashed border-white flex flex-col items-center justify-center p-12 text-center shadow-inner">
               <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center mb-6 text-[#1877F2] border-4 border-indigo-500 shadow-xl">
               ...
               <h2 className="gold-3d-text text-4xl mb-4">ဈေးသည်</h2>
               ...
               <p className="text-[10px] text-slate-400 mt-4 uppercase font-black tracking-widest opacity-60">High Quality Sales Assistant</p>
            </div>
          )}
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          <div className="w-full max-w-sm bg-white h-full relative z-[201] shadow-2xl p-8 animate-slide-left overflow-y-auto">
             <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black">Archives</h3>
                <button onClick={() => setIsHistoryOpen(false)} className="text-slate-300 hover:text-slate-900">✕</button>
             </div>
             <HistoryList history={history} onSelectItem={(item) => { setMarketingPlan(item.plan); setCurrentImageUrl(item.imageUrl); setCurrentProduct(item.productName); setIsHistoryOpen(false); }} onClear={() => setHistory([])} />
          </div>
        </div>
      )}
    </div>
